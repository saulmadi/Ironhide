...
namespace Ironhide.Users.Domain.Specs.CommandHandlers
{
    public class when_creating_a_password_reset_token
    {
        ...
        static IEventedCommandHandler<IUserSession, StartPasswordResetProcess> _handler;
        ...
        Establish context =
            () =>
            {
                var readOnlyRepository = Mock.Of<ISampleRepository>();
                ...
                var otherUser = new UserEmailLogin(Guid.NewGuid(), "Other User", "other@email.com",
        ...
        Because of =
            () => _handler.Handle(new VisitorSession(), new StartPasswordResetProcess(EmailAddress));
        ...
        It should_create_the_password_reset_token =
            () => Mock.Get(_tokenRepo)
                .Verify(
                    x =>
                        x.Create(
                            WithExpected.Object(new Sample(TokenId, _userWithMatchingEmailAddress.Id,
