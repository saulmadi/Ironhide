...
namespace Ironhide.Users.Domain.Specs.Validation
{
    public class when_validating_a_password_reset_request_where_email_does_not_match_a_user
    {
        ...
        static ISampleRepository _readOnlyRepsitory;
        static Guid _userId;

        Establish context =
            () =>
            {
                _readOnlyRepsitory = Mock.Of<ISampleRepository>();
        ...
        Because of =
            () =>
                _exception =
                    Catch.Exception(() => _validator.Validate(null, new StartPasswordResetProcess(EmailAddress)).Await());
