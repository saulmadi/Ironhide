...
{
    public class when_successfully_deleting_a_sample
    {
        ...
        Because of =
            () => _result = _browser.Delete("/sample",
                with =>
                {
                    with.HttpRequest();
        ...
        It should_dispatch_the_command =
            () => Mock.Get(_commandDispatcher)
                .Verify(
                    x => x.Dispatch(_userSession, WithExpected.Object(new DeleteSample(_request.Id), AllowAnonymous.No)));
    }
}