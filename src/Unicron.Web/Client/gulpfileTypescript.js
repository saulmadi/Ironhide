...
gulp.task('reloading_ts', function (callback) {
    var stream = runSequence('vet', 'copyingTs', callback);
    return stream;
});
gulp.task('rebuild-dev', ['clean-code', 'vet', 'copyingTs', 'copyingHtmls'], function (done) {
    console.log('Recompiling Typescript files for Dev');
    return compile_ts_with_maps(config.buildTs, config.build);
});
gulp.task('recompile-ts', ['vet', 'copyingTs'], function (done) {
    console.log('Recompiling Typescript files for Dev');
    return compile_ts_with_maps(config.buildTs, config.build);
});
...
gulp.task('reload-serve-dev', function (callback) {
    var stream = runSequence('rebuild-dev', 'inject', 'test-for-build', callback);
    return stream;
});
gulp.task('reload-ts', function (callback) {
...
gulp.task('copyingHtmls', ['lintHtmls'], function () {
    ...
    var cache = config.cache.htmlCopy;
    return gulp
...
gulp.task('vet', function () {
    ...
    var cache = config.cache.tsLinted;
    var stream = gulp
        .src(allTs)
        .pipe(plugins.cached(cache))
...
function changeEvent(event) {
    ...
    if (event.type === 'deleted') {
        ...
        _.forEach(caches, function (cacheName) {
            var deletedFile = path.basename(event.path);
            var cacheKeys = _.keys(plugins.cached.caches[cacheName]);
            var pathOfFileToDelete = _.find(cacheKeys, function (key) {
